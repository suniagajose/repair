...
class TestRepairStockMove(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # Products
        cls.product_1 = cls.env["product.product"].create({"name": "Desk Combination", "type": "product"})
        cls.product_2 = cls.env["product.product"].create({"name": "Conference Chair", "type": "product"})
        cls.product_3 = cls.env["product.product"].create({"name": "Large Cabinet", "type": "product"})
        ...
        # Replenish products
        cls.env["stock.quant"]._update_available_quantity(cls.product_1, cls.stock_location_14, 1)
        cls.env["stock.quant"]._update_available_quantity(cls.product_2, cls.stock_location_14, 1)
        cls.env["stock.quant"]._update_available_quantity(cls.product_3, cls.stock_location_14, 1)
